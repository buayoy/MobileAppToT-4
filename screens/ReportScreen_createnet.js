...
import { Alert, StyleSheet, Text, View, Image, Modal, ScrollView , NativeModules , AsyncStorage} from 'react-native';
...
export default class ReportScreen_createnet extends Component {
    ...
    _Postproblem = async () => {
        ...
        if(this.state.problemnetname.length == ''){
            ...
            Alert.alert('กรุณาเลือกตำบล');
        }else if(this.state.problemnetdetail.length == ''){
            Alert.alert('กรุณากรอกรายงานปัญหา');
        }else if(this.state.tel.length == ''){
            Alert.alert('กรุณากรอกเบอร์โทรศัพท์ติดต่อกลับ');
        }else{
            const response = await axios.post('http://npcrapi.netpracharat.com/api/problemnet/post', {
                problemnetname: this.state.problemnetname,
                problemnetdetail: this.state.problemnetdetail,
                tel: this.state.tel,
            ...
            if (response.data.data == 'ok') {
                Alert.alert('แจ้งปัญหาเรียบร้อยแล้ว', 'ปัญหาที่คุณแจ้งจะถูกบันทึกไปยังปัญหาของฉัน', [{ text: 'ตกลง' }]);
                 this.props.navigation.navigate('Report_btn_step_1');
                //this.props.navigation.reset([NavigationActions.navigate({ routeName: 'Report_myproblem' })], 0)
            } else {
    ...
    async uploadPhoto(problemnet_id) {
        //problem_id = 4;
        const response = await axios.post('http://npcrapi.netpracharat.com/api/problemnet/upload_image?problemnet_id='+problemnet_id, {
    ...
    async uploadmultiPhoto(problemnet_id) {
        // problemnet_id = 3;
        await this.state.images.forEach((item, i) => {
            const response = axios.post('http://npcrapi.netpracharat.com/api/problemnet/upload_image?problemnet_id='+problemnet_id, {
                imageData: item.uri
            });
        });
    ...
    async getproblemType() {
        const response = await axios.get('http://npcrapi.netpracharat.com/api/saveuser/getproblemtypeandroid');
        this.setState({ getproblemtype: response.data });
        // alert(JSON.stringify(response.data));
    }
  

    renderImage(image) {
        return <Image style={{margin: 10, width: 150, height: 150, resizeMode: 'contain'}} source={image} />
    }
            ...
            <View style={styles.container}>
                ...
                <KeyboardAwareScrollView>
                    ...
                    <Dropdown
                        ...
                        ref={problemnetname => this.problemnetname = problemnetname}
                        label=''
                        data={this.state.getproblemtype}
                        onChangeText={
                            problemnetname => this.setState({ problemnetname })
                        }
                    />
                    ...
                    <FormInput
                        ...
                        ref={problemnetdetail => this.problemnetdetail = problemnetdetail}
                        placeholder=''
                        onChangeText={
                            problemnetdetail => this.setState({ problemnetdetail: problemnetdetail.trim() })
                        }
